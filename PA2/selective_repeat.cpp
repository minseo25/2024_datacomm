...
queue<long> buffer_SR; // buffer for selective repeat scheme, save at most W packets (with its delay)
...
bool canRemove(long, long); // check if packet can be removed from TransitQ (delayed ACK)
...
bool canBuffer(long, long); // check if packet can be buffered
...
void initialize(void) {
    ...
    K = 2*W; // minimum number of SN in selective repeat scheme
...
void suc_transmission(long ack_sn) {
    DataQue *ptr = TransitQ_front, *next, *prev = NULL;
    AckQue *aptr;
    long suc_pk_sn;

    suc_pk_sn = (ack_sn + K -1) % K;
    while(ptr != NULL) { // buffered된 패킷들과 함께 처리하고 ACK 응답 올 수 있음. TransitQ에서의 패킷 순서 보장 x
        next = ptr->link;
        if(canRemove(suc_pk_sn, ptr->sn)) {
            if(!prev) { // if ptr == TransitQ_front
                TransitQ_front = ptr->link;
            } else {
                prev->link = ptr->link;
            }
            if(ptr == TransitQ_rear)
                TransitQ_rear = prev;
...
bool canBuffer(long next_sn, long seqn) {
    if(next_sn+W <= K) {
        if(seqn > next_sn && seqn < next_sn+W)
            return true;
        else
            return false;
    } else {
        if(seqn > next_sn || seqn < (W-(K-next_sn)))
            return true;
        else
            return false;
    }
...
void receive_pk(long seqn, double gtm) {
    if(uniform_random() > p_data) { // packet not lost
        if(next_sn == seqn) {
            ...
            while(!buffer_SR.empty()) {
                if(buffer_SR.front() == next_sn) {
                    next_sn = (next_sn+1) % K;
                    buffer_SR.pop();
                } else {
                    break;
            ...
            if(uniform_random() > p_ack) { // ACK packet not lost
                enque_Ack(next_sn); // insert ACK packet to AQ
            }
        } else if(canBuffer(next_sn, seqn)) {
            t_delay += cur_tm + t_pro - gtm;
            t_pknum++;
            buffer_SR.push(seqn);
        ...
        } else { // delayed ACK 누락되어 duplicate packet 수신된 것, ACK 재전송
            if(uniform_random() > p_ack)
                enque_Ack(next_sn);
        }
    }
}
